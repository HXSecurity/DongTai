    ...
    def test_agent_hardencode_vuln(self):
        json_ = {
            "detail": {
                ...
                False,
                "queryString":
                "domain=example.org&foo=www",
                "uri":
                "/sink/ssrf/unsafe/apache-httpclient-domain-part",
                "url":
                "http://localhost:8080/sink/ssrf/unsafe/apache-httpclient-domain-part",
                "protocol":
                "HTTP/1.1",
                "replayRequest":
                False,
        ...
        strategy = IastStrategyModel.objects.filter(
            user_id=1, vul_type='crypto-bad-mac').first()
        if strategy:
