...
def function_preheat():
    ...
    need_preheat = IastLog.objects.filter(action_time__gt=time_threshold).exists()
    if need_preheat:
        time_min = datetime.now() - timedelta(hours=72)
        user_ids = list(
            IastLog.objects.filter(action_time__gt=time_min)
...
class PreHeatRegister:
    ...
    def register(cls, function):
        ...
        if function in cls.functions:
            logger.info(f"{function.__name__} already in PreHeatRegister.functions")
            return
