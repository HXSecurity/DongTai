...
def handle_save(pk, app_label, model_name):
    ...
    rate_limit_key = f"{app_label}-{model_name}-rate_limit"
    cache.decr(rate_limit_key)


@shared_task
def handle_batch_save(app_label, model_name):
    list_key = f"{app_label}-{model_name}-task"
    rate_limit_key = f"{app_label}-{model_name}-rate_limit"
    batch_task_count_key = f"{app_label}-{model_name}-batch-task-count"
    con = get_redis_connection()
    model_ids = con.lrange(list_key, 0, DONGTAI_REDIS_ES_UPDATE_BATCH_SIZE)
    ...
    if con.llen(list_key) > 0:
        handle_batch_save.delay(app_label, model_name)
    else:
        con.decr(batch_task_count_key)
