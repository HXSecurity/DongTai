...
INSERT INTO `iast_deploy`(`id`, `desc`, `middleware`, `os`) VALUES (8, '> SpringBoot\n\n首先，下载agent.jar，然后放入具有写入权限的目录中，如：`/tmp/`\n\n然后，针对SpringBoot应用，1）如果使用**war包**的方式部署，agent的安装方式为具体中间件的安装方式；2）如果使用`java -jar app.jar`的方式部署，则直接在启动命令中增加启动参数`-javaagent:/path/to/agent.jar`即可，如：`java -javaagent:/path/to/agent.jar -jar app.jar`', 'SpringBoot', 'linux');
INSERT INTO `iast_deploy`(`id`, `desc`, `middleware`, `os`) VALUES (10, '> SpringBoot\n\n首先，下载agent.jar，然后放入具有写入权限的目录中，如：`/tmp/`\n\n然后，针对SpringBoot应用，1）如果使用**war包**的方式部署，agent的安装方式为具体中间件的安装方式；2）如果使用`java -jar app.jar`的方式部署，则直接在启动命令中增加启动参数`-javaagent:/path/to/agent.jar`即可，如：`java -javaagent:/path/to/agent.jar -jar app.jar`', 'SpringBoot', 'windows');
COMMIT;
