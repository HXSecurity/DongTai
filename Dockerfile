...
RUN mkdir /tmp/logstash/report/{img,word,pdf,excel} -p && mkdir -p /tmp/iast_cache/package && mv /opt/dongtai/*.jar /tmp/iast_cache/package/ || true && mv /opt/dongtai/*.tar.gz /tmp/ || true 
