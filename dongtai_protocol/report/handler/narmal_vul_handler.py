...
class BaseVulnHandler(IReportHandler):
    ...
    def get_vul_info(self):
        ...
        strategy_id = 0
        # 根据用户ID判断获取策略中的漏洞等级
        hook_type = HookType.objects.values(
            'id', 'enable').filter(value=vul_type).first()
        if hook_type:
            hook_type_id = hook_type.get('id', 0)
            vul_type_enable = hook_type.get('enable', 0)
            strategy = IastStrategyModel.objects.values(
                'level_id', 'id').filter(hook_type_id=hook_type_id).first()
            if strategy:
...
class NormalVulnHandler(BaseVulnHandler):
    ...
    def save(self):
        ...
        if iast_vul:
            ...
            iast_vul.full_stack = full_stack# json.dumps(self.app_caller[index + 1:])
            ...
            iast_vul = IastVulnerabilityModel.objects.create(
                ...
                full_stack=full_stack, #json.dumps(self.app_caller[index + 1:]),
