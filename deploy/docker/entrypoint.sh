...
if [ "$1" = "worker" ]; then
	...
	celery -A dongtai_conf worker -l info -Q dongtai-method-pool-scan,dongtai-replay-vul-scan,dongtai-sca-task -E --pidfile= -P gevent -c 100
elif [ "$1" = "worker-other" ]; then
	celery -A dongtai_conf worker -l info -X dongtai-periodic-task,dongtai-method-pool-scan,dongtai-replay-vul-scan,dongtai-sca-task -E --pidfile= -P gevent -c 100
elif [ "$1" = "beat" ]; then
