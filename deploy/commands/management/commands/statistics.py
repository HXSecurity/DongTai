...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        total_note_vulnerability = IastVulnerabilityModel.objects.filter(
            level_id=5).count()
        total_high_asset_vulnerability = IastAssetVul.objects.filter(
            level_id=1).count()
        total_medium_asset_vulnerability = IastAssetVul.objects.filter(
            level_id=2).count()
        total_low_asset_vulnerability = IastAssetVul.objects.filter(
            level_id=3).count()
        total_info_asset_vulnerability = IastAssetVul.objects.filter(
            level_id=4).count()
        total_note_asset_vulnerability = IastAssetVul.objects.filter(
            level_id=5).count()
