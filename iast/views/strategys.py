...
class StrategyEndpoint(UserEndPoint):
    ...
    def get(self, request):
        strategy_models = HookType.objects.values('id', 'name', 'value', 'enable').filter(type=const.RULE_SINK)
        if strategy_models:
            models = dict()
            for strategy_model in strategy_models:
                models[strategy_model['id']] = {
                    ...
                    'state': const.STRATEGY_DISABLE if strategy_model['enable'] is not None and strategy_model['enable'] == 0 else const.STRATEGY_ENABLE,
                    'vul_desc': '',
                    'level': 1,
                    'dt': 1  # 删除该字段
                }
            ...
            profiles = IastStrategyModel.objects.values('level_id', 'vul_desc', 'vul_fix', 'hook_type_id').filter(
                hook_type_id__in=strategy_ids)
            if profiles:
