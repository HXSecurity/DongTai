...
class EngineHookRuleEnableEndPoint(UserEndPoint):
    ...
    def get(self, request):
        ...
        if rule_type is not None and scope == 'all':
            count = HookStrategy.objects.filter(hooktype__id=rule_type).update(
                enable=op)
            logger.info(
                _('Policy type {} operation success, total of {} Policy types'
                  ).format(rule_type, count))
            status = True
        if hook_rule_type is not None and language_id is not None and scope == 'all':
            ...
            count = HookStrategy.objects.filter(
                hooktype__id__in=hook_type_ids, ).update(enable=op)
            logger.info(_('total of {} Policy types').format(count))
            status = True
        elif rule_id is not None:
            status = self.set_strategy_status(strategy_id=rule_id,
                                              strategy_ids=None,
                                              enable_status=op)
    ...
    def post(self, request):
        ...
        try:
            ...
            return R.failure(_("Parameter error"))
        if strategy_ids:
            count = self.set_strategy_status(strategy_id=None,
                                             strategy_ids=strategy_ids,
                                             enable_status=op)
            logger.info(
                _('Strategy operation success, total {}').format(count))
            return R.success(msg=_('Operation success'))
        else:
