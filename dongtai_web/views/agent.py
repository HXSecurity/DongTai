...
class _AgentResponseDataAgentSerializer(serializers.ModelSerializer):
    ...
    project_name = serializers.CharField(
        help_text=_("Project name, used to start the agent first and then create the project")
    )
    online = serializers.IntegerField(help_text=_("1 is running online, 0 is not running, same token, only one online"))
    project_version_id = serializers.IntegerField(
        help_text=_("Bundled project version ID, if it exists, it will be bundled"),
        default=0,
    )
    language = serializers.CharField(help_text=_("Agent language currently included in the project"))
...
class Agent(UserEndPoint):
    @extend_schema_with_envcheck(
        ...
        description=_("Delete the specified project version according to the conditions."),
